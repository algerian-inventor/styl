...
async function capture() {
  ...
  async function processUrl(url, prefix) {
    ...
    try {
      ...
      await page.waitForTimeout(4000); // Wait for redirects and images
      
      // Attempt to nuke overlays
      await page.evaluate(() => {
      ...
      const el = await page.evaluateHandle(() => {
        ...
        for (const img of imgs) {
          ...
          // Ignore small icons and likely logos (e.g. 50x50, or area < 20000)
          if (area > maxArea && rect.width > 200 && rect.height > 200 && area > 40000) {
            // also ignore if src contains 'emoji' or 'icon'
